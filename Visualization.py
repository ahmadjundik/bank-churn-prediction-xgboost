...
if attribute == 'Gender':
    ...
    if 'Card_Category' in df.columns:
        ...
        bar_fig = px.bar(churn_data, barmode='group', title="Customer Churn by Card Category",
                         ...
                         color_discrete_sequence=['#636EFA', '#EF553B'])  # Custom color scheme
        st.plotly_chart(bar_fig, use_container_width=True)
    ...
    if 'Months_on_book' in df.columns:
        ...
        hist_fig = px.histogram(df_filtered, x='Months_on_book', color='Churn Status',
                                ...
                                barmode='stack',
                                nbins=int(df['Months_on_book'].max() - df['Months_on_book'].min())  # Adjust bin size based on data
                               )
    ...
    if 'Total_Relationship_Count' in df.columns:
        relationship_selection = st.sidebar.slider('Select Total Relationship Count Range:', min_value=int(df['Total_Relationship_Count'].min()), max_value=int(df['Total_Relationship_Count'].max()), value=(int(df['Total_Relationship_Count'].min()), int(df['Total_Relationship_Count'].max())))
        df_filtered = df[(df['Total_Relationship_Count'] >= relationship_selection[0]) & (df['Total_Relationship_Count'] <= relationship_selection[1])]
        ...
        hist_fig = px.histogram(df_filtered, x='Total_Relationship_Count', color='Churn Status',
                                ...
                                barmode='stack',
                                nbins=int(df['Total_Relationship_Count'].max() - df['Total_Relationship_Count'].min())  # Adjust bin size based on data
                               )
    ...
    if 'Months_Inactive_12_mon' in df.columns:
        inactive_selection = st.sidebar.slider('Select Months Inactive 12 Mon Range:', min_value=int(df['Months_Inactive_12_mon'].min()), max_value=int(df['Months_Inactive_12_mon'].max()), value=(int(df['Months_Inactive_12_mon'].min()), int(df['Months_Inactive_12_mon'].max())))
        df_filtered = df[(df['Months_Inactive_12_mon'] >= inactive_selection[0]) & (df['Months_Inactive_12_mon'] <= inactive_selection[1])]
        st.title("Dashboard Customer Churn by Months Inactive 12 Mon")
        
        hist_fig = px.histogram(df_filtered, x='Months_Inactive_12_mon', color='Churn Status',
                                labels={'Months_Inactive_12_mon': 'Months Inactive 12 Mon', 'Churn Status': 'Churn Status'},
                                title='Customer Churn by Months Inactive 12 Mon',
                                barmode='stack',
                                nbins=int(df['Months_Inactive_12_mon'].max() - df['Months_Inactive_12_mon'].min())  # Adjust bin size based on data
                               )
        hist_fig.update_layout(xaxis_title='Months Inactive 12 Mon', yaxis_title='Count of Customers',
                               legend_title="Churn Status")
        st.plotly_chart(hist_fig, use_container_width=True)

elif attribute == 'Contacts Count 12 mon':
    if 'Contacts_Count_12_mon' in df.columns:
        contacts_selection = st.sidebar.slider('Select Contacts Count 12 Mon Range:', min_value=int(df['Contacts_Count_12_mon'].min()), max_value=int(df['Contacts_Count_12_mon'].max()), value=(int(df['Contacts_Count_12_mon'].min()), int(df['Contacts_Count_12_mon'].max())))
        df_filtered = df[(df['Contacts_Count_12_mon'] >= contacts_selection[0]) & (df['Contacts_Count_12_mon'] <= contacts_selection[1])]
        st.title("Dashboard Customer Churn by Contacts Count 12 Mon")
        
        hist_fig = px.histogram(df_filtered, x='Contacts_Count_12_mon', color='Churn Status',
                                labels={'Contacts_Count_12_mon': 'Contacts Count 12 Mon', 'Churn Status': 'Churn Status'},
                                title='Customer Churn by Contacts Count 12 Mon',
                                barmode='stack',
                                nbins=int(df['Contacts_Count_12_mon'].max() - df['Contacts_Count_12_mon'].min())  # Adjust bin size based on data
                               )
        hist_fig.update_layout(xaxis_title='Contacts Count 12 Mon', yaxis_title='Count of Customers',
                               legend_title="Churn Status")
        st.plotly_chart(hist_fig, use_container_width=True)
    ...
    if 'Credit_Limit' in df.columns:
        ...
        hist_fig = px.histogram(df_filtered, x='Credit_Limit', color='Churn Status',
                                ...
                                barmode='stack',
                                nbins=int(df['Credit_Limit'].max() - df['Credit_Limit'].min())  # Adjust bin size based on data
                               )
    ...
    if 'Total_Revolving_Bal' in df.columns:
        revolving_bal_selection = st.sidebar.slider('Select Total Revolving Balance Range:', min_value=int(df['Total_Revolving_Bal'].min()), max_value=int(df['Total_Revolving_Bal'].max()), value=(int(df['Total_Revolving_Bal'].min()), int(df['Total_Revolving_Bal'].max())))
        df_filtered = df[(df['Total_Revolving_Bal'] >= revolving_bal_selection[0]) & (df['Total_Revolving_Bal'] <= revolving_bal_selection[1])]
        ...
        hist_fig = px.histogram(df_filtered, x='Total_Revolving_Bal', color='Churn Status',
                                ...
                                barmode='stack',
                                nbins=int(df['Total_Revolving_Bal'].max() - df['Total_Revolving_Bal'].min())  # Adjust bin size based on data
                               )
    ...
    if 'Total_Trans_Amt' in df.columns:
        trans_amt_selection = st.sidebar.slider('Select Total Transaction Amount Range:', min_value=int(df['Total_Trans_Amt'].min()), max_value=int(df['Total_Trans_Amt'].max()), value=(int(df['Total_Trans_Amt'].min()), int(df['Total_Trans_Amt'].max())))
        df_filtered = df[(df['Total_Trans_Amt'] >= trans_amt_selection[0]) & (df['Total_Trans_Amt'] <= trans_amt_selection[1])]
        ...
        hist_fig = px.histogram(df_filtered, x='Total_Trans_Amt', color='Churn Status',
                                ...
                                barmode='stack',
                                nbins=int(df['Total_Trans_Amt'].max() - df['Total_Trans_Amt'].min())  # Adjust bin size based on data
                               )
    ...
    if 'Total_Trans_Ct' in df.columns:
        trans_ct_selection = st.sidebar.slider('Select Total Transaction Count Range:', min_value=int(df['Total_Trans_Ct'].min()), max_value=int(df['Total_Trans_Ct'].max()), value=(int(df['Total_Trans_Ct'].min()), int(df['Total_Trans_Ct'].max())))
        df_filtered = df[(df['Total_Trans_Ct'] >= trans_ct_selection[0]) & (df['Total_Trans_Ct'] <= trans_ct_selection[1])]
        ...
        hist_fig = px.histogram(df_filtered, x='Total_Trans_Ct', color='Churn Status',
                                ...
                                barmode='stack',
                                nbins=int(df['Total_Trans_Ct'].max() - df['Total_Trans_Ct'].min())  # Adjust bin size based on data
                               )
        hist_fig.update_layout(xaxis_title='Total Transaction Count', yaxis_title='Count of Customers',
                               legend_title="Churn Status")
        st.plotly_chart(hist_fig, use_container_width=True)

elif attribute == 'Avg Utilization Ratio':
    if 'Avg_Utilization_Ratio' in df.columns:
        utilization_selection = st.sidebar.slider('Select Avg Utilization Ratio Range:', min_value=float(df['Avg_Utilization_Ratio'].min()), max_value=float(df['Avg_Utilization_Ratio'].max()), value=(float(df['Avg_Utilization_Ratio'].min()), float(df['Avg_Utilization_Ratio'].max())))
        df_filtered = df[(df['Avg_Utilization_Ratio'] >= utilization_selection[0]) & (df['Avg_Utilization_Ratio'] <= utilization_selection[1])]
        st.title("Dashboard Customer Churn by Avg Utilization Ratio")
        
        hist_fig = px.histogram(df_filtered, x='Avg_Utilization_Ratio', color='Churn Status',
                                labels={'Avg_Utilization_Ratio': 'Avg Utilization Ratio', 'Churn Status': 'Churn Status'},
                                title='Customer Churn by Avg Utilization Ratio',
                                barmode='stack',
                                nbins=50  # Adjust bin size based on data
                               )
